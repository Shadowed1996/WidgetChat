(function () {
  ...
  const SCHEMA = [
    ...
    },
    {
      ...
      aiuto: 'Windows ha un interruttore che chiede a tutti i programmi di muoversi il meno possibile. Sta in tre posti diversi e conviene saperli tutti e tre, perché cercarlo dove non c’è fa concludere che il problema sia un altro: su Windows 11 è Impostazioni → Accessibilità → Effetti visivi → «Effetti di animazione», su Windows 10 è Impostazioni → Accessibilità → Schermo → «Mostra animazioni in Windows», e lo spegne anche «Regola per ottenere le prestazioni migliori» nelle opzioni prestazioni di sistema, che è la prima spunta che si tocca su un computer da gioco. Serve a chi sta male a guardare le cose che si muovono, e il pollaio lo rispetta: con «come dice il computer», se quell’interruttore è spento non si anima più niente. Non solo l’effetto d’ingresso: anche la fascia dell’hype train, il pallino della spia e il pollo che dondola. C’è però un caso in cui quella risposta è alla domanda sbagliata. Dentro una sorgente browser di OBS il computer non sta guardando, sta disegnando: la preferenza è di chi trasmette, ma l’immagine la guardano gli spettatori, che non l’hanno mai espressa. Lì metto «anima comunque». Sulla macchina che uso per guardare la chat, no.'
    },
    {
      gruppo: 'aspetto',
      chiave: 'larghezza',
